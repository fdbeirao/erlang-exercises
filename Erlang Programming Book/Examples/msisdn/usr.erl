...
start(FileName) ->
  ...
  receive 
    started -> ok 
  after ?TIMEOUT -> {error, starting} end.
...
handle_request({add_usr, PhoneNo, CustId, Plan}) ->
  usr_db:add_usr(#usr{msisdn=PhoneNo, id=CustId, plan=Plan});

handle_request({delete_usr, CustId}) ->
  usr_db:delete_usr(CustId);

handle_request({set_service, CustId, Service, Flag}) ->
...
handle_request({set_status, CustId, Status}) ->
  case usr_db:lookup_id(CustId) of
    {ok, Usr} ->
...
handle_request(delete_disabled) ->
